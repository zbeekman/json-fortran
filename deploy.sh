...
# to github.
set -xev # Echo what we're doing, debug and fail on any errors
if [ ! "$TRAVIS" ]; then #not on travis, try a sane deploy of current branch's documentation
	...
	if [ "$TRAVIS_BRANCH" = "master" ] || [ "$TRAVIS_BRANCH" = "auto-deploy" ] && [ "$(ls -A $TRAVIS_BUILD_DIR/documentation)" ] ; then #not empty
